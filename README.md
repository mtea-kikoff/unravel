...
**Consolidate several PRs at once.** Paste multiple thread links into the search box (space- or newline-separated) and Unravel extracts all of them into a single brief, grouped by PR with every finding tagged `PR #N`. Non-PR threads in the mix are noted and skipped. Handy when a stack of related PRs needs its fixes handed to one coding-agent session.
