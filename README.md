...
**Consolidate several PRs at once.** Click **+ Consolidate reviews from multiple PRs** to open a panel with one input box per link (add or remove boxes as needed), then **Extract & consolidate**. Unravel merges them all into a single brief, grouped by PR with every finding tagged `PR #N`. Non-PR threads in the mix are noted and skipped. If a PR spans two email threads (GitHub sometimes splits notifications), pasting both links merges them under one PR section — every thread's findings are kept, and only findings identical across threads collapse. Handy when a stack of related PRs needs its fixes handed to one coding-agent session.
